...
def handle_comment(comment):
    ...
    comment.reply(
        f"{user.name} was first active in r/{sub.display_name} on {utc_to_human_readable(first_activity.created_utc)} [here](https://reddit.com{first_activity.permalink}). Since then, they have made {len(comments)} comments and {len(submissions)} submissions. They have made an average of {activity_per_day:0.2f} comments and submissions per day."
    )
